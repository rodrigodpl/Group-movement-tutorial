...
bool EntityManager::IsOccupied(iPoint tile, Unit* ignore_unit) {
	...
	// TODO 1 Create a function that checks if there's already a unit in the tile
	// remember to ignore the unit that called the method!
...
void EntityManager::OnCollision(Collider * c1, Collider * c2)
{
	...
	if (unit_to_move->state == UNIT_MOVING && unit2->state == UNIT_IDLE) {
		...
		// TODO 4: Use your freshly finished FindNearestAvailable() to provide an available tile for the colliding unit.
		// remember to pop first the front node if the path is not empty
	}
}
